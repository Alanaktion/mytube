<template>
    <Menu as="div" class="relative self-center mb-4 sm:mb-0" v-slot="{ open }">
        ...
        >
            <MenuItems
                class="origin-top-right absolute -left-2 sm:left-auto sm:-right-2 w-48 p-2 mt-2 z-30 flex flex-col gap-1 rounded-lg shadow-lg bg-white dark:bg-trueGray-800 border dark:border-trueGray-850 focus:outline-none"
            >
                <MenuItem v-slot="{ active }" v-for="file in files" :key="file.id">
                    <a
                        ...
                        :class="[
                            ...
                            'flex justify-between items-center appearance-none w-full px-4 py-2 text-sm cursor-pointer rounded-md',
                        ]"
                        download
                    >
                        <span v-if="file.height">{{ file.height }}p {{ file.mime_type.substring(6).toUpperCase() }}</span>
                        <span v-else>{{ file.mime_type.substring(6).toUpperCase() }}</span>
