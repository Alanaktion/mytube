<template>
    <div
        class="px-3 py-2 mb-4 flex items-center gap-2 bg-blue-100 dark:bg-blue-900 bg-opacity-50 border border-blue-100 dark:border-blue-600 p-3 rounded"
        v-for="item in details"
        :key="item.id"
    >
        <svg class="animate-spin h-5 w-5 text-blue-500 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <a
            v-if="item.model"
            class="text-blue-700 dark:text-blue-300 cursor-pointer truncate"
            :href="modelHref(item)"
        >
            {{ item.model.title }}
        </a>
        <div
            ...
            class="relative flex items-stretch bg-white dark:bg-trueGray-900 rounded overflow-hidden w-32 h-5 ml-auto shadow-sm shrink-0"
            role="progressbar"
            :aria-valuemax="item.data.count"
            :aria-valuenow="item.data.imported"
        >
            <div class="bg-blue-300 dark:bg-blue-700" :style="{ width: `${item.data.imported / item.data.count * 100}%` }" />
