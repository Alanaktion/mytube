            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <x-card>
                    @if ($tokens->count() > 0)
                            ...
                            @foreach($tokens as $token)
                                <div class="flex items-center py-2 lg:py-4 border-b dark:border-neutral-700 last:border-0">
                                    ...
                                    @else
                                        <div>Never used</div>
                                    @endif
                                    <form class="ml-4" action="{{ route('tokens.destroy', [auth()->id(), $token->id]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <x-slot name="footer">
                            <form class="flex mt-1" action="{{ route('tokens.store', auth()->id()) }}" method="POST">
                                @csrf
                    ...
                    @else
                        <div class="py-8 lg:py-12 flex flex-col items-center">
                            ...
                            <form class="flex gap-3 mt-1" action="{{ route('tokens.store', auth()->id()) }}" method="POST">
                                @csrf
                                <label for="token-name" class="sr-only">Token name</label>
