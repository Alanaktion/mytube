        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div>
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-slate-500 bg-white border border-slate-300 dark:text-neutral-500 dark:bg-neutral-800 dark:border-neutral-700 rounded-l-md leading-5 hover:text-primary-500 dark:hover:text-neutral-400 focus:z-10 focus:outline-none focus:ring ring-slate-300 dark:ring-neutral-600 focus:border-primary-300 active:bg-slate-100 active:text-slate-500 dark:active:bg-neutral-700 dark:active:text-neutral-500" aria-label="{{ __('pagination.previous') }}">
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-white text-shadow-px-primary bg-gradient-to-b from-primary-500 to-primary-600 border border-primary-500 cursor-default leading-5">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-slate-700 bg-white border border-slate-300 dark:text-neutral-400 dark:bg-neutral-800 dark:border-neutral-700 leading-5 hover:text-primary-500 dark:hover:text-neutral-300 focus:z-10 focus:outline-none focus:ring ring-slate-300 dark:ring-neutral-600 focus:border-primary-300 active:bg-slate-100 active:text-slate-700 dark:active:bg-neutral-700 dark:active:text-neutral-400" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
                    ...
                    {{-- Next Page Link --}}
                    @if ($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-slate-500 bg-white border border-slate-300 dark:text-neutral-500 dark:bg-neutral-800 dark:border-neutral-700 rounded-r-md leading-5 hover:text-primary-500 dark:hover:text-neutral-400 focus:z-10 focus:outline-none focus:ring ring-slate-300 dark:ring-neutral-600 focus:border-primary-300 active:bg-slate-100 active:text-slate-500 dark:active:bg-neutral-700 dark:active:text-neutral-500" aria-label="{{ __('pagination.next') }}">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
